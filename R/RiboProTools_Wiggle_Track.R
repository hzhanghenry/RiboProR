...
#*		annot_bed_file:	GRanges of gene/transcript annotation.
#*
#*
#*		Return:
#*
...
getWiggleCounts <- function(bam_file, asite_table, annot_bed_file=NULL)
{
	...
	for(a_chrom in 1:nrow(chrom_length))
	{
		...
		all_counts[[chrom_name]] <- countAtAsite(
			alignments[chrom_index], chrom_len, asite_table);	
	}
...
#*		5.	countAtAsite()
#*
#*		Count read number overlap at A-site for transcripts on one 
#*		chromosome. The overlap is stranded.
#*
#*		Arguments:
#*
...
{
	asite_table <- read.table(a_site_file, header=TRUE, sep="\t",
						quote="");
...
#'		Last revised on January 7, 2019
#'
getBigWiggleFile <- function(bam_file, transcripts)
{
...
#'								file in bed format.
#'
#'		Return:		GRange object of filtered alignments.
#'
#'
#'		Last revised on January 7, 2019
#'
filterOutAlignments <- function(alignments, asite_table, 
							annot_bed_file)
{
	...
	if(!is.null(annot_bed_file))
		alignments <- getTranscriptReadsOnly(alignments, 
				annot_bed_file);
...
#'								file in bed format.
#'
#'		Return:		GRange object of all alignments.
#'
#'		Last revised on January 7, 2019
#'
...
#'								row names.
#'
#'		Return:		GRange object of alignments overlapped with transcript
#'					regions only.
#'
#'		Last revised on January 7, 2019
#'
...
#'		norm_method: character vector, one of "raw", "scale", or "cpm". 
#'		scale_to:	Positive integer, scale total counts to this level.
#'					Default: 1000000000
#'
#'		Return:		None. Result will be write to file.
#'
#'		Last modified on June 03, 2020
#'

getBigBigFiles <- function(bam_file=NULL, bam_type="Riboseq", 
	...
	scale_to=1000000000)
{
	...
	if(bam_type == "riboseq") {
		asite <- readAsiteFromFile(a_site_file);
	...
	if(norm_method == "scale") {
		...
		export(scaled_asite_cov, con=sub("bam", "scales.bw", 
		...
		export(cpm_asite_cov,  con=sub("bam", "cpm.bw", 
				basename(bam_file)), "BW");
	} else if (norm_method == "raw") {
		export(asite_cov, con=sub("bam", "bw", 
			basename(bam_file)), "BW");
